...
/**
 * The test vases ate same as full-test, but run on the v2 contract
 */
describe("TheDAONFTv2", function () {
    ...
    it("Should set asset URLs and admin", async function () {
        ...
        // in v2 event Curator(address curator);
        // becomes event OwnershipTransferred(address previousOwner, address newOwner);
        expect(await nft.setCurator(owner.address)).to.emit(nft, 'OwnershipTransferred').withArgs(owner.address, owner.address);
    ...
    it("It should transfer NFTs", async function () {
        ...
        expect(stats[8]).to.be.equal(peth('0')); // in v2 3 becomes 8
